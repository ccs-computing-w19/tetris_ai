...
def display(game, window):
    ...
    window.addstr("\n  " + "".join([("[]" if (-1, i, False) in game.PIECES[game.next-1] or (-1, i, True) in game.PIECES[game.next-1] else "  ") for i in range(4)]) + " turns: " + str(game.numTurns))
    window.addstr("\n  " + "".join([("[]" if (0, i, False) in game.PIECES[game.next-1] or (0, i, True) in game.PIECES[game.next-1] else "  ") for i in range(4)]) + " lost: " + str(game.lost))
    window.addstr("\n  " + "".join([("[]" if (1, i, False) in game.PIECES[game.next-1] or (1, i, True) in game.PIECES[game.next-1] else "  ") for i in range(4)]) + " clears: " + str(game.numLines))
    window.addstr("\n  " + "".join([("[]" if (2, i, False) in game.PIECES[game.next-1] or (2, i, True) in game.PIECES[game.next-1] else "  ") for i in range(4)]) + " next: " + str(game.next))
